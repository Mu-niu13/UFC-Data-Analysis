...
ufc_data <- ufc_data %>%
  ...
  filter(!is.na(Outcome), !is.na(TotalSubAttempts), !is.na(WeightClass))  # Remove missing data

# Step 2: Logistic Regression by Weight Class
results <- ufc_data %>%
  group_by(WeightClass) %>%
  summarise(
    model = list(glm(Outcome ~ TotalSubAttempts, data = cur_data(), family = binomial))
  ) %>%
  mutate(
    summary = lapply(model, summary),
    tidy_model = lapply(model, tidy)
  )

# Extract and print summary for each weight class
for (i in seq_along(results$WeightClass)) {
  cat("\nWeight Class:", results$WeightClass[i], "\n")
  print(results$summary[[i]])
}

# Combine tidy outputs for a summarized table
tidy_results <- bind_rows(
  lapply(seq_along(results$WeightClass), function(i) {
    results$tidy_model[[i]] %>%
      mutate(WeightClass = results$WeightClass[i])
  })
)

print(tidy_results)

# Step 3: Visualize Results by Weight Class
# Odds Ratios
tidy_results <- tidy_results %>%
  filter(term == "TotalSubAttempts") %>%
  mutate(
...
ggplot(tidy_results, aes(x = WeightClass, y = OddsRatio)) +
  geom_point() +
  geom_errorbar(aes(ymin = LowerCI, ymax = UpperCI), width = 0.2) +
  labs(
    title = "Odds Ratios for Total Submission Attempts by Weight Class",
    x = "Weight Class",
